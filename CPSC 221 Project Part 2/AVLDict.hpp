...
class AVLDict : public PredDict
  ...
  private:
	...
	void balance(node *&x);
	void insert(MazeState* key, MazeState* pred, node *&root);
	bool find_helper(string keyID, node *root, MazeState *&pred, int depth);
	node* createNode(MazeState* key, MazeState* pred);
